...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for ticket in orm.Ticket.objects.all():
          ticket.owner_id = ticket.rider.id
          ticket.owner_type = orm['contenttypes.ContentType'].objects.filter(model='riderinfo')[0]
          ticket.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        ri = orm['contenttypes.ContentType'].objects.filter(model='riderinfo')[0]
        for ticket in orm.Ticket.objects.all():
          if ticket.owner_type == ri:
            ticket.rider = orm.RiderInfo.objects.get(pk=ticket.owner_id)
            ticket.save()
          else:
            raise RuntimeError("Cannot reverse customer tickets")
