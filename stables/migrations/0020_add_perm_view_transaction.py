...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        ct, created = orm['contenttypes.ContentType'].objects.get_or_create(model='transaction', app_label='stables')
        perm, created = orm['auth.permission'].objects.get_or_create(content_type=ct, codename='view_saldo')
        if created:
            perm.name=u"Can see saldos"
            perm.save()
        perm, created = orm['auth.permission'].objects.get_or_create(content_type=ct, codename='view_transaction')
        if created:
            perm.name=u"Can see transactions"
            perm.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        ct, created = orm['contenttypes.ContentType'].objects.get_or_create(model='transaction', app_label='stables')
        perm = orm['auth.permission'].objects.filter(content_type=ct, codename='view_saldo')
        perm.delete()
        perm = orm['auth.permission'].objects.filter(content_type=ct, codename='view_transaction')
        perm.delete()
    ...
    models = {
        ...
        'stables.ticket': {
            ...
            'expires': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 11, 28, 22, 52, 53, 734829)', 'null': 'True', 'blank': 'True'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
